          ...
          >
            <template v-slot:[`item.symbol_code`]="{ item }">
              <div class="d-flex align-center">
                <v-col
                  cols="auto"
                  class="px-0 float-left font-weight-bold"
                >
                  {{ item.symbol.code }}
                </v-col>
                <v-spacer />
            ...
            <template v-slot:[`item.average`]="{ item }">
              <div class="d-flex">
                ...
                <v-col
                  cols="auto"
                  class="float-right overline text-caption"
                >
            ...
            <template v-slot:[`body.append`]>
              <tr>
                ...
                <td colspan="4" />
                <td class="text-end">
                  {{ portfolio.sum_amount }}
                </td>
                <td class="text-end">
                  {{ portfolio.sum_average_amount }}
                </td>
                <td class="text-end">
                  {{ portfolio.sum_gain }}
                </td>
                <td class="text-end">
                  {{ $n(portfolio.total_gain_percent, 'percent') }}
                </td>
              </tr>
            </template>
            <template
              v-if="portfolio.shares.length"
              #footer
            >
              <div
                class="pl-4 py-5 d-flex align-center text-caption"
                style="border-top: thin solid hsla(0,0%,100%,.12);"
              >
                <v-icon
                  x-small
                  dense
            ...
            >
              <template v-for="(item, index) in itemDetails">
                <item-detail
                  ...
                  :base-link="`portfolios/${portfolio.id}`"
                />
                <v-divider
