...
<template>
  <v-footer app absolute inset padless class="pl-4 text-caption font-weight-light">
    <v-menu bottom left offset-y max-height="calc(100% - 16px)" transition="slide-y-reverse-transition">
      <template v-slot:activator="{ attrs, on }">
        <v-btn class="text-capitalize text--secondary" small text
          v-bind="attrs"
          v-on="on"
        >
          ...
          />
          <v-icon small class="ml-1 hidden-sm-and-down" right>
            arrow_drop_up
          </v-icon>
        </v-btn>
      </template>
